...
"""
GenLayer Bradbury deployment adapter for MediChain (v2 - improved consensus).

Changes from v1:
- submit_results uses prompt_comparative instead of run_nondet_unsafe
  for LLM-based integrity analysis, which is more tolerant of LLM
  output variation across validators.
"""
...
class MediChain(gl.Contract):
    ...
    def __init__(self, treasury_address: Address):
        ...
        self.trial_ids_json = "[]"
        self.report_exists = TreeMap()
        self.report_trial_id = TreeMap()
        self.report_publication_url = TreeMap()
        self.report_integrity_score = TreeMap()
        self.report_flags_json = TreeMap()
        self.report_endpoints_match = TreeMap()
        self.report_sample_size_consistent = TreeMap()
        self.report_verdict = TreeMap()
        self.report_confidence = TreeMap()
        self.report_summary = TreeMap()
        self.flag_exists = TreeMap()
        self.flag_trial_id = TreeMap()
        self.flag_submitter = TreeMap()
        self.flag_description = TreeMap()
        self.flag_evidence_url = TreeMap()
        self.flag_status = TreeMap()

    def _require_owner(self) -> None:
        if gl.message.sender_address != self.owner:
            _fail("only the contract owner can call this method")
    ...
    def _trial_as_dict(self, trial_id: str) -> dict:
        return {
            ...
            "integrity_score": self.trial_integrity_score[trial_id],
    ...
    def list_trials(self) -> dict:
        ...
        for trial_id in _load_json_list(self.trial_ids_json):
            result[trial_id] = self._trial_as_dict(trial_id)
        return result
