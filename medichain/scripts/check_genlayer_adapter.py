...
def main() -> int:
    ...
    assert "gl.nondet.web.render(" not in source, (
        "Bradbury web rendering causes leader timeouts; documents must be "
    ...
    assert "_validate_document_snapshot(" in source
    ...
    submit_method = source.split("def submit_results(", 1)[1].split(
        "@gl.public.write",
        1,
    )[0]
    assert "current_registry_snapshot_json: str" in submit_method
    assert "publication_snapshot_json: str" in submit_method
    assert "preprint_snapshot_json: str" in submit_method
    assert "gl.nondet.web" not in submit_method
